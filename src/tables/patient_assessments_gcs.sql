-- SOURCE: https://github.com/MIT-LCP/mimic-code/blob/main/mimic-iv/concepts_duckdb/measurement/gcs.sql
...
WITH base AS (
  ...
  LEFT JOIN base AS b2
    ...
    AND b.rn = b2.rn + 1
    AND CAST(b2.charttime AS TIMESTAMP) > CAST(b.charttime AS TIMESTAMP) - INTERVAL '6' HOUR
), gcs_stg AS (
  SELECT
    subject_id,
    gs.stay_id,
